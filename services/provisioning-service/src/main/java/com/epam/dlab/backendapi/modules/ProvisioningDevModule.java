 ...
 */
public class ProvisioningDevModule extends ModuleBase<ProvisioningServiceApplicationConfiguration> {
	...
	private RESTService createAuthenticationService() {
		...
		when(result.post(eq(SecurityAPI.LOGIN), any(), any())).then(invocationOnMock -> Response.ok(TOKEN).build());
		when(result.post(eq(SecurityAPI.GET_USER_INFO), eq(TOKEN), eq(UserInfo.class)))
